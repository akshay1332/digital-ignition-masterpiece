        ...
        <div className="flex justify-between items-center h-16">
          ...
          <div className="flex-shrink-0">
            <Link to="/" className="group">
              <h2 className="text-2xl font-montserrat font-bold text-white group-hover:text-eco-green transition-colors duration-300">
              ...
              <NavigationMenuList className="space-x-4">
                {navItems.map((item) => (
                  <NavigationMenuItem key={item.name}>
                    {item.dropdown ? (
                      <>
                        <NavigationMenuTrigger className={`bg-transparent text-white hover:text-eco-green transition-colors duration-300 px-3 py-2 text-sm font-roboto font-medium ${isActive(item.href) ? 'text-eco-green' : ''}`}>
                          {item.name}
                        </NavigationMenuTrigger>
                        <NavigationMenuContent>
                          <div className="w-[300px] p-4 bg-dark/95 border border-eco-green/20 rounded-lg">
                            {item.dropdown.map((dropdownItem) => (
                              <NavigationMenuLink key={dropdownItem.name} asChild>
                                <Link
                                  to={dropdownItem.href}
                                  className={`block px-3 py-2 text-white hover:text-eco-green transition-colors duration-300 rounded ${isActive(dropdownItem.href) ? 'text-eco-green bg-eco-green/10' : ''}`}
                                >
                                  {dropdownItem.name}
                                </Link>
                              </NavigationMenuLink>
                      ...
                      <NavigationMenuLink asChild>
                        <Link
                          to={item.href}
                          className={`text-white hover:text-eco-green transition-colors duration-300 px-3 py-2 text-sm font-roboto font-medium relative group ${isActive(item.href) ? 'text-eco-green' : ''}`}
                        >
                          {item.name}
          ...
          <div className="hidden md:block">
            <Link to="/contact">
              <Button className="bg-gradient-to-r from-eco-green to-eco-dark text-dark font-roboto font-medium hover:shadow-lg hover:shadow-eco-green/25 transition-all duration-300">
